...
SELECT COUNT(MR.REVIEW_NO) AS REVIEW_COUNT
FROM MOVIE_REVIEW MR
JOIN MOVIE M ON M.MOVIE_NO = MR.MOVIE_NO
JOIN USER_INFO UI ON UI.USER_NO = MR.USER_NO
LEFT JOIN REPORT R ON R.REVIEW_NO = MR.REVIEW_NO;



SELECT MR.REVIEW_NO, MR.SCORE, TO_CHAR(MR.REVIEW_DATE, 'YYYY-MM-DD') AS REVIEW_DATE,
       MR.REVIEW_CONTENT, MR.USER_NO, MR.REVIEW_DELETE_FL, M.MOVIE_TITLE, UI.USER_NICKNAME,
       LISTAGG(R.REPORT_NO, ', ') WITHIN GROUP (ORDER BY R.REPORT_NO) AS REPORT_NO_LIST
FROM MOVIE_REVIEW MR
JOIN MOVIE M ON M.MOVIE_NO = MR.MOVIE_NO
JOIN USER_INFO UI ON UI.USER_NO = MR.USER_NO
LEFT JOIN REPORT R ON R.REVIEW_NO = MR.REVIEW_NO
GROUP BY MR.REVIEW_NO, MR.SCORE, MR.REVIEW_DATE, MR.REVIEW_CONTENT,
         MR.USER_NO, MR.REVIEW_DELETE_FL, M.MOVIE_TITLE, UI.USER_NICKNAME
ORDER BY MR.REVIEW_NO DESC;

SELECT * FROM REPORT;	

SELECT R.REPORT_NO,R.REPORT_TITLE,R.REPORT_CONTNENT,R.REPORT_ENROLL_DATE,R.REVIEW_NO,R.REPORT_WIRTER,R.REPORTED_USER_NO,MR.REVIEW_DELETE_FL 
FROM REPORT R
JOIN MOVIE_REVIEW MR ON R.REVIEW_NO = MR.REVIEW_NO
ORDER BY REPORT_NO DESC;

SELECT R.REPORT_NO, R.REPORT_TITLE, R.REPORT_CONTNENT, R.REVIEW_NO, R.REPORT_WIRTER, UI.USER_ID , R.REPORTED_USER_NO , UI.USER_ID
FROM REPORT R
JOIN MOVIE_REVIEW MR ON R.REVIEW_NO = MR.REVIEW_NO
JOIN USER_INFO UI ON UI.USER_NO = R.REPORTED_USER_NO
ORDER BY R.REPORT_NO DESC;

SELECT R.REPORT_NO, R.REPORT_TITLE, R.REPORT_CONTNENT, R.REVIEW_NO, UI1.USER_ID AS REPORT_WIRTER, UI2.USER_ID AS REPORTED_USER_NO,MR.REVIEW_DELETE_FL
FROM REPORT R
JOIN MOVIE_REVIEW MR ON R.REVIEW_NO = MR.REVIEW_NO
...
SELECT R.REPORT_NO,R.REPORT_TITLE,R.REPORT_CONTNENT,R.REVIEW_NO,UI1.USER_ID AS REPORT_WIRTER, UI2.USER_ID AS REPORTED_USER_NO,M.REVIEW_DELETE_FL
FROM REPORT R
JOIN MOVIE_REVIEW M ON R.REVIEW_NO = M.REVIEW_NO
JOIN USER_INFO UI1 ON UI1.USER_NO = R.REPORT_WIRTER
JOIN USER_INFO UI2 ON UI2.USER_NO = R.REPORTED_USER_NO;
		...
		USER_INFO.USER_NICKNAME,QNA.QNA_DELETE_FL
		FROM QNA 
		JOIN USER_INFO ON QNA.USER_NO2 = USER_INFO.USER_NO ;
...
ADD REPORTED_COUNT INT DEFAULT 0;

COMMENT ON COLUMN "MOVIE_REVIEW"."REPORTED_COUNT" IS '신고된 리뷰';
...
SELECT * FROM QNA;
	...
	SELECT USER_NO,
		...
		QNA.QNA_DELETE_FL,
		QNA.QNA_CHECK_FL,
		QNA_IMG.QNA_IMG_PATH,QNA.NONMEMBER_NAME,QNA.NONMEMBER_EMAIL,QNA.NONMEMBER_TEL
		FROM QNA
		LEFT JOIN USER_INFO ON QNA.USER_NO2 = USER_INFO.USER_NO
		LEFT JOIN (
		SELECT QNA_NO, MAX(QNA_IMG_PATH) AS QNA_IMG_PATH
...
SELECT * FROM REPORT;
	...
	WHERE M.REVIEW_DELETE_FL = 'Y';
		...
		FROM QNA 
		LEFT JOIN USER_INFO ON QNA.USER_NO2 = USER_INFO.USER_NO 
		WHERE QNA.QNA_NO LIKE '%128%';
		
	
		SELECT
			 QNA.QNA_NO,QNA.QNA_TITLE,TO_CHAR(QNA.QNA_ENROLL_DATE, 'YYYY-MM-DD') AS QNA_ENROLL_DATE,
			USER_INFO.USER_NICKNAME,  USER_INFO.USER_ID,
			QNA.QNA_DELETE_FL,QNA.QNA_CHECK_FL,qc.QNA_COMMENT_FL 
			FROM QNA
			LEFT JOIN USER_INFO ON QNA.USER_NO2 = USER_INFO.USER_NO
			LEFT JOIN "QNA _COMMENT" qc  ON QNA.QNA_NO  = QC .QNA_NO 
		...
		SELECT
			...
			FROM QNA
			JOIN USER_INFO ON QNA.USER_NO2 = USER_INFO.USER_NO
			JOIN "QNA _COMMENT" qc  ON QNA.QNA_NO  = QC .QNA_NO 
			ORDER BY QNA.QNA_NO DESC;
	...
	SELECT QNA_COMMENT_NO,QNA_COMMENT_DATE,QNA_COMMENT_FL,USER_NO,QNA_NO,QNA_COMMENT
		FROM "QNA _COMMENT"
		WHERE QNA_COMMENT_NO = '172';
		
	SELECT * FROM "QNA _COMMENT" WHERE QNA_NO = '31';
	SELECT * FROM QNA q;


SELECT USER_NO,
		QNA.QNA_NO,
		QNA.QNA_TITLE,QNA.QNA_CATEGORY,QNA.QNA_CONTENT ,
		TO_CHAR(QNA.QNA_ENROLL_DATE, 'YYYY-MM-DD') AS QNA_ENROLL_DATE,
		USER_INFO.USER_NICKNAME,USER_INFO.USER_ID,
		QNA.QNA_DELETE_FL,
		QNA.QNA_CHECK_FL,
