...
const imgBox = document.querySelector(".my-page-myInquiry-imgBox");

for (let i = 0; i < up_down_img.length; i++) {

    up_down_img[i].addEventListener("click", (e) => {
        const qnaNo = e.currentTarget.parentElement.parentElement.children[0].innerText;
        const thisImgBox = e.currentTarget.parentElement.parentElement.nextElementSibling.nextElementSibling.querySelector('.my-page-myInquiry-imgBox');
        console.log(qnaNo);

        console.log(thisImgBox);

        fetch("/myPage/my-page-inquiry/detail?qnaNo=" + qnaNo)
            .then((resp) => resp.json())
            .then((detailList) => {
                ...
                if (detailList != null && !thisImgBox.querySelector('img')) {
                    for (let j = 0; j < detailList.length; j++) {
                        const img = document.createElement('img');
                        img.src = detailList[j].qnaImgPath+detailList[j].qnaImgRename;
                        thisImgBox.appendChild(img);
